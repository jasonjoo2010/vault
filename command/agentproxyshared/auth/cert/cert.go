...
func (c *certMethod) AuthClient(client *api.Client) (*api.Client, error) {
	...
	if c.caCert != "" || (c.clientKey != "" && c.clientCert != "") {
		...
		if hash, err := hashCert(c.clientCert, c.clientKey); err != nil {
			return nil, err
		} else {
			c.latestHash = &hash
		}
...
func hashCert(certFile, keyFile string) (string, error) {
	...
	sum := sha1.Sum(append(certPEMBlock, keyPEMBlock...))
...
func (c *certMethod) runWatcher() {
	...
	for {
		...
		case <-c.ticker.C:
			hash, err := hashCert(c.clientCert, c.clientKey)
		...
		if changed {
			c.logger.Debug("The cert/key files changed")
			select {
