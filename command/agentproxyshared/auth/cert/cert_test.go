...
func TestHashCert(t *testing.T) {
	...
	sum, err = hashCert("./test-fixtures/keys/cert.pem", "./test-fixtures/keys/key.pem")
	if err != nil {
		t.Fatal("hashCert shouldn't fail with valid cert/key pair")
	}
	sum1 := sum

	sum, err = hashCert("./test-fixtures/root/rootcacert.pem", "./test-fixtures/root/rootcakey.pem")
	if err != nil {
		t.Fatal("hashCert shouldn't fail with valid cert/key pair")
	}
	if sum == sum1 {
		t.Fatal("hashCert shouldn't return the same checksum between different pairs")
	}
}
